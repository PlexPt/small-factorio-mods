...
common.dwrite("mod_list: " .. serpent.block(mod_list))
...
common.dwrite("new mod_list: " .. serpent.block(mod_list))
...
local function check_version(name, needed)
    ...
    -- Mod is active
    if found then
        common.dwrite(string.format("Checking mod \"%s\": Found version %s, require version %s",
                name, found, needed))
        ...
        if version[1] < needed[1] then
            common.dwrite(string.format("Major number is too small: %g < %g", version[1], needed[1]))
            ret = false

            -- Major number is greater: Hit!
        elseif version[1] > needed[1] then
            common.dwrite(string.format("Major number is greater: %g > %g", version[1], needed[1]))
            ret = true

            -- Major number is same, minor number is greater: Hit!
        elseif version[2] > needed[2] then
            common.dwrite(string.format("Minor number is greater: %g > %g", version[2], needed[2]))
            ret = true
        ...
        elseif version[2] == needed[2] and version[3] >= needed[3] then
            common.dwrite(string.format("Least number is greater or equal: %g >= %g", version[3], needed[3]))
            ret = true

            -- Version is smaller than required
        else
            common.dwrite(string.format("Least number is too small: %g < %g", version[3], needed[3]))
            ret = false
        end

        -- Mod isn't active
    else
        common.dwrite(string.format("Mod \"%s\" is not active!", name))
        ret = false
    end
...
for name, version in pairs(mod_list) do
    if check_version(name, version) then
        common.dwrite(string.format("\"%s\" is active! Won't overwrite vanilla character and corpse.",
                name))
...
for e, entities in ipairs({ char, corpse }) do
    ...
    if replace_base_char then
        common.dwrite(string.format("Stand-alone mode: Overwriting vanilla \"%s\" with \"%s\"",
        ...
        common.dwrite(string.format("Keeping both vanilla \"%s\" and \"%s\"!", entities[name].type, name))
        ...
        common.dwrite(string.format("Added localization for \"%s\"!", name))

        -- Make sure the Gear Girl won't have the default character's corpse!
        if c then
            entities[name].character_corpse = name .. "-corpse"
            common.dwrite(string.format("Activated new corpse for \"%s\": \"%s\"!",
